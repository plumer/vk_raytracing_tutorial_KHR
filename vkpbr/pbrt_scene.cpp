...
glm::vec3 ParseSpectrum(const std::vector<float>& numbers, const std::string& key)
{
    if (contains(key, "rgb")) {
        ...
        auto sspec =
            SampledSpectrum::FromSampled(lambdas.data(), values.data(), cast_i32(lambdas.size()));
    ...
    if (impl == "mirror") {
        ...
        for (auto& parameter : params) {
            if (contains(parameter.key, "Kr")) {
                Kr_tex =
                    ConstantOrImageTexture(parameter.numbers, parameter.strings, glm::vec3(0.9f));
            } else {
        ...
        for (auto p : non_bumpmap_parameters) {
            if (contains(p.key, "Kd")) {
                diffuse_tex = ConstantOrImageTexture(p.numbers, p.strings, glm::vec3(0.5f));
        ...
        for (auto p : params) {
            if (contains(p.key, "Kd")) {
                res.diffuse_tex_id = ConstantOrImageTexture(p.numbers, p.strings, glm::vec3(0.25f));
            } else if (contains(p.key, "Ks")) {
                res.specular_tex_id =
                    ConstantOrImageTexture(p.numbers, p.strings, glm::vec3(0.25f));
