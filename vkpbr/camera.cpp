...
CameraNavigator::Actions CameraNavigator::MouseMove(int x, int y, const Inputs& inputs) {
    ...
    if (inputs.left_mouse) {
        ...
        action = Actions::kPan;
        //LOG(INFO) << "Middle mouse panning: " << Format("x = %d, y = %d", x, y);
...
void CameraNavigator::Motion(int x, int y, Actions action) {
    // Filters out NoAction and prevents the current mouse position from being updated to by an
    // undefined value.
    if (action == Actions::kNoAction)
        return;
    ...
    //LOG(INFO) << __FUNCTION__ << Format(" (x,y)=(%d,%d), delta=(%f,%f)", x, y, dx*1000, dy*1000);
    ...
    switch (action) {
        ...
        case Actions::kNoAction:
            LOG(FATAL) << "no action is illegal";
            break;
        default:
...
void CameraNavigator::Pan(float dx, float dy) {
    if (mode_ == Modes::kFly) {
        dx *= -1;
        dy *= -1;
        LOG(INFO) << "Fly mode in effect";
    }

    //LOG(INFO) << Format("dx = %f, dy = %f", dx, dy);
...
std::string InertiaCamera::to_string() const {
    ...
    sprintf(buffer, "[%5.2f, %5.2f, %5.2f] => [%5.2f, %5.2f, %5.2f]", eye_.x, eye_.y, eye_.z,
