    ...
    const vk::AccelerationStructureCreateInfoKHR& accel_struct_ci) const
{
    ...
    // Allocates the buffer to hold the accel structure.
    result_accel.buffer =
        MakeBuffer(accel_struct_ci.size, vk::BufferUsageFlagBits::eAccelerationStructureStorageKHR
                                             | vk::BufferUsageFlagBits::eShaderDeviceAddress);

    // Sets the buffer.
    auto new_ci = accel_struct_ci;
    new_ci.buffer     = result_accel.buffer.handle;
    result_accel.handle = device_.createAccelerationStructureKHR(new_ci);
